...
def parse_model_run_datestamp(key: str) -> Tuple[str, str]:
    '''
    Parse the model run date from the key of the file in the NOS S3 bucket,
    given the NOS naming convention:
...
def parse_model_run_datestamp_offset(key: str) -> Tuple[str, int]:
    '''
    Parse the model run forecast time key from the key of the file in the
    NOS S3 bucket, given the NOS naming convention:
        'nos/nos.dbofs.fields.f001.20230315.t00z.nc.zarr'
    where the model_date is 20230315 and the model_hour is 00 and the offset is
    1, this would result in a key of 20230315T01
    '''
    offset, model_date, model_hour = re.search(
        r'f(\d{3}).(\d{8}).t(\d{2})', key).groups()
    model_date = datetime.datetime.strptime(
        f'{model_date}T{model_hour}', '%Y%m%dT%H') + datetime.timedelta(hours=int(offset))
...
def generate_model_run_glob_expression(key: str, model_date: str, model_hour: str) -> str:
    ...
    The following expression will be created:
        'nos/nos.dbofs.fields.f*.{model_date}.t{model_hour}z.nc.zarr'
    '''
    prefix, postfix = re.search(
        r'(.*).f\d{3}.\d{8}.t\d{2}z.(.*)', key).groups()
...
def generate_best_time_series_glob_expression(key: str) -> str:
    ...
    Parse the glob prefix and postfix given the zarr single file key:
        'nos/nos.dbofs.fields.f001.20230315.t00z.nc.zarr'
    The following expression will be created:
        'nos/nos.dbofs.fields.f*.*.t*z.nc.zarr'
    '''
    prefix, postfix = re.search(
        r'(.*).f\d{3}.\d{8}.t\d{2}z.(.*)', key).groups()
...
def generate_kerchunked_nc(region: str, bucket: str, key: str, dest_bucket: str, dest_prefix: str):
    ...
    fs_read = fsspec.filesystem(
        's3', anon=True, skip_instance_cache=True, use_ssl=False)
    fs_write = fsspec.filesystem(
        's3', anon=False, skip_instance_cache=True, use_ssl=False)
...
def generate_kerchunked_model_run(region: str, bucket: str, key: str):
    '''
    Generate or update the multizarr kerchunked aggregation for
    the model run that the specified file belongs to
    '''
    try:
        model_date, model_hour = parse_model_run_datestamp(key)
        model_run_glob = generate_model_run_glob_expression(
            key, model_date, model_hour)
    except Exception as e:
        print(
            f'Failed to parse model run date and hour from key {key}: {e}. Skipping...')
        return
    ...
    print(
        f'Aggregating {model_run_file_count} model files for model run {model_date} t{model_hour}z...')
    ...
    mzz = MultiZarrToZarr(
        ...
        identical_dims=['eta_rho', 'xi_rho', 's_rho', 'eta_psi', 'xi_psi', 's_w', 'eta_u', 'xi_u',
                        'eta_v', 'xi_v', 'lat_rho', 'lat_psi', 'lat_u', 'lat_v', 'lon_rho', 'lon_psi', 'lon_u', 'lon_v']
    )
...
def generate_kerchunked_best_time_series(region: str, bucket: str, key: str):
    '''
    Generate or update the best time series kerchunked aggregation for the
    model run. If the specified file is not in the best time series,
    ...
    if target_key not in model_best_files:
        print(
            f'{key} is not a part of the current best time series for its model. Skipping...')
        return

    model_run_file_count = len(model_best_files)
    print(
        f'Aggregating {model_run_file_count} model files for best time series aggregation...')
...
def parse_nos_sqs_message(sqs_payload: str) -> Tuple[str, str, str]:
    ...
    needed for kerchunking and uploading to a destination bucket
    '''
...
def generate_kerchunked_nos_sqs(sqs_payload: str, dest_bucket: str, dest_prefix: str, key_filter: str = None):
    '''
    Generate a kerchunked zarr file from a netcdf file in s3 given an
    SQS payload from the NOS S3 bucket
    '''
...
def generate_kerchunked_nos_multizarr_sqs(sqs_payload: str):
    '''
    Given an SQS payload from the NOS S3 bucket, generate or update the
    multizarr kerchunked aggregation for the model run that the changed file
    belongs to, and update the best time series if the target key is in the
    best time series
    '''
    region, bucket, key = parse_nos_sqs_message(sqs_payload)
...
if __name__ == '__main__':
    ...
    if command == 'dump':
        ...
        generate_kerchunked_nos_sqs(
            sqs_payload, dest_bucket, dest_prefix, key_filter)
